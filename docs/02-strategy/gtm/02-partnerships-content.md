...
# 1Dragon Go-to-Market 전략: 파트너십 & 콘텐츠 & 한국 특화
...
| **공동 마케팅** | TikTok x 1Dragon 공동 웨비나 "셀러를 위한 숏폼 마케팅 마스터클래스" |
| **타임라인** | Phase 2 (M8~M10) |
...
| **3. 튜토리얼 / 팁** | 활성화 + 리텐션 | 20% | "1Dragon 사용법", "상품 사진 잘 찍는 법", "숏폼 트렌드" |
...
| 항목 | 전략 |
|------|------|
| **핵심 전략** | 1Dragon로 만든 영상을 직접 게시 -> "이 영상은 사진 1장으로 만들었습니다" |
...
| **시리즈** | (1) 업종별 성공 사례 (2) 1Dragon 기능 튜토리얼 (3) 숏폼 마케팅 마스터클래스 |
...
| **1Dragon 공식 커뮤니티** | 사용자 간 영상 공유, 피드백, 기능 요청 | 상시 (Phase 2~) |
...
| **M1-W2** | "스마트스토어 셀러, 영상 마케팅 시작하는 법" | "How 1Dragon Works: Technical Deep Dive" | 패션 상품 쇼케이스 3개 | 튜토리얼 #1: 첫 영상 만들기 | 아이보스 사례 공유 |
| **M1-W3** | "상품 사진으로 TikTok 영상 만들기 (실전 가이드)" | - | 뷰티 상품 쇼케이스 3개 | - | 스마트스토어 카페 체험기 |
| **M1-W4** | "AI 영상 vs 수동 제작, 광고 성과 비교" | "1Dragon vs Manual: A/B Test Results" | 식품 상품 쇼케이스 3개 | 케이스 스터디 #1: OO 셀러 | - |
...
```mermaid
graph TD
    A[네이버 스마트스토어<br/>활성 셀러 30~35만] --> B[1Dragon 연동<br/>상품 이미지 자동 인입]
    B --> C[네이버 클립 영상 생성<br/>검색/블로그 연동 최적화]
    C --> D[네이버 쇼핑라이브<br/>숏폼 클립 노출]
    D --> E[검색 노출 가산점<br/>영상 있는 상품 > 없는 상품]

    F[네이버 블로그 SEO] --> G[1Dragon 인지도 확대]
    G --> A
```

| 전략 | 구체적 실행 | 기대 효과 |
|------|-----------|----------|
| **스마트스토어 API 연동** | 판매자센터에서 "1Dragon로 영상 만들기" 버튼. 상품 이미지/설명/가격 자동 인입 | 셀러의 TTV를 30초로 단축. 워크플로우 임베딩 |
...
| **로켓그로스 셀러 타겟** | 로켓그로스 셀러 대상 특화 캠페인. 물류는 쿠팡, 마케팅은 1Dragon | 번들 가치 제안 |
