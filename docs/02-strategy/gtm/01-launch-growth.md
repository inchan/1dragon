...
| H3 | "Made with SnapVid" 워터마크 포함 영상의 바이럴 계수(K-factor)가 0.3 이상이다 | K-factor | >= 0.3 | 워터마크 포함 영상 -> 서비스 유입 추적 (UTM + 레퍼럴 코드) | MVP 8주차 |
...
| H6 | 에이전시가 SnapVid 도입 시 클라이언트당 영상 제작 원가를 40% 이상 절감한다 | 제작 원가 절감률 | >= 40% | 파일럿 에이전시 5곳의 Before/After 비용 분석 | Phase 2 중반 |
...
| Before/After 쇼케이스 영상 | 10편 | 실제 상품 사진 -> SnapVid 생성 영상 비교. 업종별(패션, 뷰티, 식품, 악세서리, 가전) |
...
| **TikTok/Instagram** | SnapVid로 만든 영상을 광고 소재로 활용 (메타 광고) | CPI(Cost Per Install) 기반 효율 측정 |
...
| **바이럴 워터마크** | "Made with SnapVid" 포함 시 월 5개 추가 무료 영상 | K-factor 0.3 -> 0.5+ 목표. PLG 유기적 성장 엔진 |
...
|------|-------------|---------|---------|------|
| **PLG 바이럴 (워터마크)** | 1인 셀러, 인플루언서 | 0 (오가닉) | P0 | 영상 내 "Made with SnapVid" -> 시청자 유입. 워터마크 포함 시 월 5개 추가 무료 |
...
| **TikTok 광고** | 1인 셀러, 인플루언서 | 3,000~10,000 | P1 | SnapVid로 만든 영상 자체가 광고 소재. "이 영상, 사진 1장으로 만들었습니다" |
...
| **ROI 시각화** | "이번 달 SnapVid로 절약한 시간: 12시간, 금액: 360,000" 대시보드 |
...
| 메커니즘 | 설계 | K-factor 기여 |
|---------|------|-------------|
| **제품 내재 바이럴** | 생성 영상이 SNS에 올라가는 순간 "Made with SnapVid" 노출 | 수동적, 지속적 |
