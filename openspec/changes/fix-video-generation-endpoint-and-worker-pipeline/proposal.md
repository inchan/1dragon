...
현재 SnapVid는 핵심 기능(영상 생성)의 사용성 검증이 되지 않을 정도로 실행 경로가 끊어진 상태입니다. 특히 API 진입점과 워커 파이프라인이 서로 연결되지 않아 실제 생성 요청을 처리하지 못하고, 배포 환경에서도 헬스체크로 서비스 상태를 빠르게 판정하기 어렵습니다.
